"""
Main script for Instagram Reels Generator with pyttsx3 integration.
This is the modified entry point that uses pyttsx3 for TTS and enables perfect audio-visual sync.
"""
'''
import os
import time
...
def main():
    ...
    # Adjust segment durations to target 30 second total video length
...
def main(custom_script=None):
    ...
    Parameters:
    - custom_script: Optional custom script generated via Grok API
    """
    ...
    if custom_script:
        story_script = custom_script
        print(f"Using custom script: {story_script['title']}")
    else:
    ...
    if not custom_script:
        story_script = adjust_segment_duration(story_script, target_total_duration=30)
    else:
        # Make sure all segments have subtitle_mode enabled
        for segment in story_script["segments"]:
            segment["subtitle_mode"] = True
            
        # For narrative stories with 8 segments in 1 minute
        if len(story_script["segments"]) == 8 and abs(sum(s.get("duration_seconds", 7.5) for s in story_script["segments"]) - 60) < 5:
            print(f"Using 8-segment narrative format for 1-minute video")
            # Leave durations as-is since they're already set for a 1-minute video
        else:
            # Adjust segment durations to fit desired length
            total_duration = sum(s.get("duration_seconds", 7.5) for s in story_script["segments"])
            target_duration = 60 if len(story_script["segments"]) >= 8 else 30
            
            if abs(total_duration - target_duration) > 5:  # Only adjust if significantly different
                print(f"Adjusting segment durations to target {target_duration} seconds")
                story_script = adjust_segment_duration(story_script, target_total_duration=target_duration)

    # Save the script to file
    script_path = f"{base_dir}/1_script/story_script.json"
    with open(script_path, 'w') as f:
        json.dump(story_script, f, indent=4)
    ...
    # Generate narration for each segment
    for i, segment in enumerate(story_script["segments"]):
        print(f"Generating narration for segment {i+1}/{len(story_script['segments'])}")
    ...
    # Generate audio for each segment
    for i, segment in enumerate(story_script["segments"]):
        print(f"Generating audio for segment {i+1}/{len(story_script['segments'])}")
    ...
    try:
        ...
        for i, segment in enumerate(story_script["segments"]):
            print(f"Generating image for segment {i+1}/{len(story_script['segments'])}")
        ...
        for i, segment in enumerate(story_script["segments"]):
            print(f"Creating placeholder image for segment {i+1}/{len(story_script['segments'])}")
    ...
    for i, image_path in enumerate(segment_images):
        ...
        # Get segment details
        segment = story_script["segments"][i]
    ...
    # Create transition frames for each pair of segments
    for i in range(len(story_script["segments"]) - 1):
        from_segment = i + 1
        to_segment = i + 2
        transition_type = story_script["segments"][i].get("transition", "smooth_fade")

        # Use actual audio duration for perfect transitions
        segment_duration = story_script["segments"][i].get("audio_duration", 
                          story_script["segments"][i]["duration_seconds"])
    ...
    if success:
        ...
        if custom_script and len(story_script["segments"]) == 8:
            print("\nComplete narrative story video generation process finished!")
            print(f"Final video available at: {final_video}")
            print("\nThis video features:")
            print("- Custom narrative storytelling")
            print("- Professional pyttsx3 voice narration")
            print("- Synchronized captions and audio")
